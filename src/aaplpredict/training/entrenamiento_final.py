...
# Directorio donde se guardan los modelos para la API
...
def log_mlflow(nombre: str, params: dict, metricas: dict, modelo, scaler=None) -> None:
    try:
        ...
        print(f"  ⚠️  MLflow no disponible, se omite el registro [{nombre}]: {e}")


def guardar_modelo_local(nombre: str, modelo, scaler=None) -> None:
    """Guarda el modelo en models/ para que la API lo cargue sin MLflow."""
    if nombre == "XGBoost":
        modelo.save_model(MODELS_DIR / "xgboost_model.ubj")
    else:
        modelo.save(MODELS_DIR / f"{nombre.lower()}_model.keras")
        if scaler is not None:
            joblib.dump(scaler, MODELS_DIR / f"{nombre.lower()}_scaler.pkl")
    print(f"  Modelo guardado en models/ [{nombre}]")


def grafica_comparativa(resultados: list) -> None:
...
if __name__ == "__main__":
    ...
    for nombre in ["XGBoost", "MLP", "LSTM"]:
        ...
        guardar_modelo_local(nombre, modelo, scaler)
